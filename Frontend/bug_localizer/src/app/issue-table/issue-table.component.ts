...
export class IssueTableComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['title', 'number', 'actions'];
  dataSource: any;
  ...
  listIssues: GitModel[] = [];
  getGitIssues(): void {
    this.fileService.getAllIssue().subscribe(event => {
      this.listIssues = event.items;
      this.dataSource = new MatTableDataSource(this.listIssues);
      this.dataSource.paginator = this.paginator;
